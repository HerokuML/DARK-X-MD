...
console.log("DARK-X-MD Session downloaded ✅")
})})}
...
console.log("Connecting DARK-X-MD...⏳️");
...
console.log('DARK-X-MD Is Installing...🔥 ')
...
if (config.ANTI_LINK == "true"){
        ...
        if(groupAdmins.includes(sender)) return
            let repoInfo = `📌Link Detected DARK-X-MD📌`;
...
if (config.ANTI_LINK == "true"){
        ...
        if (isMe) return await reply("Link Derect but I cant Delete link")
        if(groupAdmins.includes(sender)) return
            let repoInfo = `📌 *Link Detected by DARK-X-MD* 📌\n⚠️❗ *Anti link on....* ⚠️❗`;
        await conn.sendMessage(from, { text: repoInfo }, { quoted: mek });  
        }}}
...
app.get("/", (req, res) => {
res.send("hey I am alive, DARK-X-MD Is started✅");
});
